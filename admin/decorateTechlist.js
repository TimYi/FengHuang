...
function getDatas(){
	getData(DECORATE_TECH,param,afterGetDatas);
}
...
function bindData(data){
	...
	dataModel.remove = function(item){
		if(ConfDel(0)){
			var url = genUrl(DECORATE_TECH)+'/'+item.id();
	...
	dataModel.modify = function(item){
		window.location.href='decorateTechedit.htm?id='+item.id();
	}
...
function add(){
		window.location.href="decorateTechadd.htm";
}
function reorder(){
		/*var idStr = '';
		var brands = dataModel.result();
		...
		param += "}";
		//提交
		var url = genUrl(DECORATE_TECH)+'/order';
