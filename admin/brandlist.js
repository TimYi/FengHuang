...
function getDatas(){
	getData(BRAND_TYPE,param,afterGetDatas);
}
...
function bindData(data){
	...
	dataModel.remove = function(item){
		...
		if(ConfDel(0)){
			...
			var url = genUrl(BRAND_TYPE)+'/'+item.id();
	...
	dataModel.modify = function(item){
		window.location.href='brandTypeedit.htm?id='+item.id();
	}	
	if(!bind){
		bind = true;
...
function add(){
		window.location.href="brandTypeadd.htm";
}