...
function getDatas(){
	getData(MUSEUM_SIGNUP,param,afterGetDatas);
}
...
function bindData(data){	
	...
	dataModel.remove = function(item){
		...
		if(ConfDel(0)){			
			var url = genUrl(MUSEUM_SIGNUP)+'/'+item.id();  
	...
	dataModel.modify = function(item){		
		...
		window.location.href='signupedit.htm?id='+item.id();		
	}
	if(!bind){
		bind = true;
...
function add(){
		window.location.href="signupadd.htm";
}