...
function getDatas(){
	getData(MUSEUM_MUSEUM,param,afterGetDatas);
}
...
function bindData(data){
	...
	dataModel.remove = function(item){		
		if(ConfDel(0)){
			var url = genUrl(MUSEUM_MUSEUM)+'/'+item.id();
	...
	dataModel.modify = function(item){
		window.location.href='experienceMuseumedit.htm?id='+item.id();
	}
	if(!bind){
		bind = true;
...
function add(){
		window.location.href="experienceMuseumadd.htm";
}